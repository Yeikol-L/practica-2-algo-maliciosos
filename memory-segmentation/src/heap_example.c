...
int main(int argc, char *argv[]) {
    ...
    if (char_ptr == NULL) { // Verificación de errores, en caso de que malloc() falle
    ...
    int_ptr = (int *)malloc(12); // Asignando memoria en el heap nuevamente
    if (int_ptr == NULL) { // Verificación de errores, en caso de que malloc() falle
        fprintf(stderr, "Error: no se pudo asignar memoria en el heap.\n");
        exit(-1);
    }
    *int_ptr = 31337; // Poner el valor de 31337 donde apunta int_ptr.
    ...
    char_ptr = (char *)malloc(15); // Asignando más memoria en el heap
    if (char_ptr == NULL) { // Verificación de errores, en caso de que malloc() falle
        fprintf(stderr, "Error: no se pudo asignar memoria en el heap.\n");
        exit(-1);
    }
